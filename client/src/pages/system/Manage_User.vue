<template>
    <div class="p-3">
        <div class="flex mb-4 justify-between items-center">
            <h1 class=" font-bold text-2xl "><i class="fa-solid fa-list"></i> Members List</h1>
                    ...
                    <form class="mt-4 space-y-4" @submit.prevent="sendInsertUsers">
                        <div class="grid grid-cols-2">
                            <div class="mb-1">
                                ...
                                <input type="email" placeholder="Enter Email" v-model="dataUser.email"
                                    class=" px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-white" />
                            </div>

                            <div class="mb-1">
                                <label class="block text-gray-700 font-medium mb-2">Password :</label>
                                <input type="text" placeholder="Enter Password" v-model="dataUser.password"
                                    class=" px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-white" />
                            </div>
                        </div>
                        <div class="mb-4">
                            <label class="block text-gray-700 font-medium mb-2">Username :</label>
                            <input type="text" placeholder="Enter Username" v-model="dataUser.username"
                                class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-white" />
                        </div>
            ...
            <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                ...
                <tbody v-if="users" class="">
                    <tr v-for="user in users" :key="user.id"
                        ...
                        <td class="px-6 py-4">
                            <div class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-1 rounded-xl">
                                {{ user.role }}
                            </div>
                        </td>
